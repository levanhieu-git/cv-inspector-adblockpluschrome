...
function labelElementBlockedAndUpdateAttempt(element, increment_blocked_attempt) {
...
function hideElement(element)
{
  function doHide()
  {
    ...
    if (element.style.getPropertyValue(propertyName) != propertyValue ||
      ...
      // LABEL element as abp-blocked-element
      labelElementBlockedAndUpdateAttempt(element, true);
  }
      ...
      nodes: for (let node of nodes)
      {
        ...
        for (let element of elements)
        {
          ...
          if (getComputedStyle(element).display == "none")
          {
            ...
            // label the blocked and attempt
            for (let tempEl of elements) {
                // LABEL element as abp-blocked-element
                labelElementBlockedAndUpdateAttempt(tempEl, false);
            }
...
ContentFiltering.prototype = {
  addRulesInline(rules, groupName = "standard", appendOnly = false)
  {
    ...
    if (!style)
    {
      ...
      style.setAttribute("abp-stylesheet", "true");
...
if (document instanceof HTMLDocument)
{
  ...
  // hard code updating of list event. To trigger this, inject JS into the page
  document.addEventListener("update_filter_lists", event => {
    console.log("update_filter_lists");
    browser.runtime.sendMessage({
